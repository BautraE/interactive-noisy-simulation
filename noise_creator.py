...
class NoiseCreator:
    def __init__(self) -> None:
        ...
        print(f"----- Creating {self.__class__.__name__} object -----")
        print(f"{self.__class__.__name__} class object has been created successfully!\n")
    ...
    def __add_readout_error(self, qubit_nr, columns) -> None:
        ...
        self.__noise_model["noise_model"].add_readout_error(readout_error, [qubit_nr])
        ...
        for instruction in CONFIG["single_qubit_gates"]:
            if CSV_COLUMNS[instruction]["csv_name"] in self.__noise_data.columns:
                ...
                if not pandas.isna(error_data):
                    ...
                    self.__noise_model["noise_model"].add_quantum_error(error, CSV_COLUMNS[instruction]["code_name"], [qubit_nr])
            ...
            if CSV_COLUMNS[instruction]["csv_name"] in self.__noise_data.columns:
                ...
                if not pandas.isna(error_data):
                    ...
                    for target_qubit in error_data.keys():
                        ...
                        self.__noise_model["noise_model"].add_quantum_error(error, CSV_COLUMNS[instruction]["code_name"], [qubit_nr, target_qubit])
    ...
    def __add_thermal_error(self, qubit_nr, columns) -> None:
        ...
        for instruction in CONFIG["single_qubit_gates"]:
            if CSV_COLUMNS[instruction]["csv_name"] in self.__noise_data.columns:
                self.__noise_model["noise_model"].add_quantum_error(thermal_error_1q, CSV_COLUMNS[instruction]["code_name"], [qubit_nr], warnings=False)
        ...
        if not pandas.isna(two_qubit_gate_time):
            for target_qubit in two_qubit_gate_time.keys():
                ...
                for instruction in CONFIG["two_qubit_gates"]:
                    if CSV_COLUMNS[instruction]["csv_name"] in self.__noise_data.columns:
                        self.__noise_model["noise_model"].add_quantum_error(thermal_err_2q, CSV_COLUMNS[instruction]["code_name"], [qubit_nr, target_qubit], warnings=False)
        ...
        self.__noise_model["noise_model"].add_quantum_error(thermal_err_readout, "measure", [qubit_nr])
        ...
        self.__noise_model["noise_model"].add_quantum_error(thermal_err_reset, "reset", [qubit_nr])
